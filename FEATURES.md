# PDF Embed & SEO Optimize - Complete Feature Matrix

<p align="center">
  <img src="https://pdfviewer.drossmedia.de/wp-content/uploads/2025/03/wp-pdf-embed-and-seo-optimized.png" alt="PDF Embed & SEO Optimize" width="100">
</p>

A comprehensive PDF management solution for WordPress, Drupal, and React/Next.js.

**Current Version:** 1.2.11 | **[Get Premium](https://pdfviewer.drossmedia.de)** | **[Documentation](./DOCUMENTATION.md)**

---

## Platform Support

| Platform | Free | Premium | Package/Module |
|----------|:----:|:-------:|----------------|
| WordPress 5.8+ | ✓ | ✓ | `pdf-embed-seo-optimize` |
| Drupal 10/11 | ✓ | ✓ | `pdf_embed_seo` |
| React 18+ | ✓ | ✓ | `@pdf-embed-seo/react` |
| Next.js 13/14/15 | ✓ | ✓ | `@pdf-embed-seo/react` |

---

## Quick Overview

| | Free | Premium |
|---|:---:|:---:|
| **Price** | $0 | From $49/year |
| **Platforms** | WordPress, Drupal, React | WordPress, Drupal, React |
| **Support** | Community | Priority Email/Chat |
| **Updates** | Lifetime | 1 Year |

---

## Feature Comparison Tables

### Viewer & Display

| Feature | Free | Premium | WP | Drupal | React |
|---------|:----:|:-------:|:--:|:------:|:-----:|
| Mozilla PDF.js Viewer (v4.0) | ✓ | ✓ | ✓ | ✓ | ✓ |
| Light Theme | ✓ | ✓ | ✓ | ✓ | ✓ |
| Dark Theme | ✓ | ✓ | ✓ | ✓ | ✓ |
| Responsive Design | ✓ | ✓ | ✓ | ✓ | ✓ |
| Print Control (per PDF) | ✓ | ✓ | ✓ | ✓ | ✓ |
| Download Control (per PDF) | ✓ | ✓ | ✓ | ✓ | ✓ |
| Configurable Viewer Height | ✓ | ✓ | ✓ | ✓ | ✓ |
| Page Navigation | ✓ | ✓ | ✓ | ✓ | ✓ |
| Zoom Controls | ✓ | ✓ | ✓ | ✓ | ✓ |
| Full Screen Mode | ✓ | ✓ | ✓ | ✓ | ✓ |
| iOS/Safari Print Support | ✓ | ✓ | ✓ | ✓ | ✓ |
| Gutenberg Block | ✓ | ✓ | ✓ | - | - |
| PDF Viewer Block | ✓ | ✓ | - | ✓ | - |
| PdfViewer Component | ✓ | ✓ | - | - | ✓ |
| Shortcodes | ✓ | ✓ | ✓ | - | - |
| **Text Search in Viewer** | - | ✓ | ✓ | ✓ | ✓ |
| **Bookmark Navigation** | - | ✓ | ✓ | ✓ | ✓ |

### Content Management

| Feature | Free | Premium | WP | Drupal | React |
|---------|:----:|:-------:|:--:|:------:|:-----:|
| PDF Document Post Type/Entity | ✓ | ✓ | ✓ | ✓ | - |
| Title, Description, Slug | ✓ | ✓ | ✓ | ✓ | ✓ |
| File Upload & Management | ✓ | ✓ | ✓ | ✓ | - |
| Featured Image/Thumbnail | ✓ | ✓ | ✓ | ✓ | ✓ |
| Auto-Generate Thumbnails | ✓ | ✓ | ✓ | ✓ | - |
| Published/Draft Status | ✓ | ✓ | ✓ | ✓ | - |
| Owner/Author Tracking | ✓ | ✓ | ✓ | ✓ | - |
| Admin List with Columns | ✓ | ✓ | ✓ | ✓ | - |
| Quick Edit Support | ✓ | ✓ | ✓ | - | - |
| Multi-language Support | ✓ | ✓ | ✓ | ✓ | ✓ |
| Media Library Integration | ✓ | ✓ | ✓ | ✓ | - |
| **Categories Taxonomy** | - | ✓ | ✓ | ✓ | ✓ |
| **Tags Taxonomy** | - | ✓ | ✓ | ✓ | ✓ |
| **Role-Based Access Control** | - | ✓ | ✓ | ✓ | - |
| **Bulk Edit Actions** | - | ✓ | ✓ | ✓ | - |
| **Bulk Import (CSV/ZIP)** | - | ✓ | ✓ | ✓ | - |

### SEO & URLs

| Feature | Free | Premium | WP | Drupal | React |
|---------|:----:|:-------:|:--:|:------:|:-----:|
| Clean URL Structure (`/pdf/slug/`) | ✓ | ✓ | ✓ | ✓ | ✓ |
| Auto Path/Slug Generation | ✓ | ✓ | ✓ | ✓ | - |
| Schema.org DigitalDocument | ✓ | ✓ | ✓ | ✓ | ✓ |
| Schema.org CollectionPage | ✓ | ✓ | ✓ | ✓ | ✓ |
| Schema.org BreadcrumbList | ✓ | ✓ | ✓ | ✓ | ✓ |
| Yoast SEO Integration | ✓ | ✓ | ✓ | - | - |
| OpenGraph Meta Tags | ✓ | ✓ | ✓ | ✓ | ✓ |
| Twitter Card Support | ✓ | ✓ | ✓ | ✓ | ✓ |
| PdfSeo Component | ✓ | ✓ | - | - | ✓ |
| **XML Sitemap (`/pdf/sitemap.xml`)** | - | ✓ | ✓ | ✓ | - |
| **Sitemap XSL Stylesheet** | - | ✓ | ✓ | ✓ | - |
| **Search Engine Ping** | - | ✓ | ✓ | ✓ | - |

### GEO/AEO/LLM Optimization

| Feature | Free | Premium | Schema Property |
|---------|:----:|:-------:|-----------------|
| Basic Speakable Schema | ✓ | ✓ | `speakable` (WebPage) |
| potentialAction (Read/Download) | ✓ | ✓ | `potentialAction` |
| accessMode & accessibilityFeature | ✓ | ✓ | `accessMode` |
| **AI Summary (TL;DR)** | - | ✓ | `abstract` |
| **Key Points & Takeaways** | - | ✓ | `ItemList` |
| **FAQ Schema (FAQPage)** | - | ✓ | `FAQPage` |
| **Table of Contents Schema** | - | ✓ | `hasPart` |
| **Reading Time Estimate** | - | ✓ | `timeRequired` |
| **Difficulty Level** | - | ✓ | `educationalLevel` |
| **Document Type Classification** | - | ✓ | `additionalType` |
| **Target Audience** | - | ✓ | `audience` |
| **Custom Speakable Content** | - | ✓ | `speakable` (custom) |
| **Related Documents** | - | ✓ | `isRelatedTo` |
| **Prerequisites** | - | ✓ | `coursePrerequisites` |
| **Learning Outcomes** | - | ✓ | `teaches` |

### Archive & Listing

| Feature | Free | Premium | WP | Drupal | React |
|---------|:----:|:-------:|:--:|:------:|:-----:|
| Archive Page (`/pdf/`) | ✓ | ✓ | ✓ | ✓ | ✓ |
| PdfArchive Component | ✓ | ✓ | - | - | ✓ |
| Pagination Support | ✓ | ✓ | ✓ | ✓ | ✓ |
| Grid Display Mode | ✓ | ✓ | ✓ | ✓ | ✓ |
| List Display Mode | ✓ | ✓ | ✓ | ✓ | ✓ |
| Sorting Options | ✓ | ✓ | ✓ | ✓ | ✓ |
| Search Filtering | ✓ | ✓ | ✓ | ✓ | ✓ |
| Visible Breadcrumb Navigation | ✓ | ✓ | ✓ | ✓ | ✓ |
| Full-Width Layout (No Sidebars) | ✓ | ✓ | ✓ | ✓ | ✓ |
| Custom Archive Heading | ✓ | ✓ | ✓ | ✓ | - |
| Content Alignment Options | ✓ | ✓ | ✓ | ✓ | - |
| Custom Font/Background Colors | ✓ | ✓ | ✓ | ✓ | - |
| usePdfList Hook | ✓ | ✓ | - | - | ✓ |
| **Category Filter** | - | ✓ | ✓ | ✓ | ✓ |
| **Tag Filter** | - | ✓ | ✓ | ✓ | ✓ |
| **Archive Page Redirect** | - | ✓ | ✓ | ✓ | - |

### REST API

| Endpoint | Free | Premium | WP | Drupal | React |
|----------|:----:|:-------:|:--:|:------:|:-----:|
| `GET /documents` (list) | ✓ | ✓ | ✓ | ✓ | Client |
| `GET /documents/{id}` (single) | ✓ | ✓ | ✓ | ✓ | Client |
| `GET /documents/{id}/data` (secure URL) | ✓ | ✓ | ✓ | ✓ | Client |
| `POST /documents/{id}/view` (track) | ✓ | ✓ | ✓ | ✓ | Client |
| `GET /settings` | ✓ | ✓ | ✓ | ✓ | Client |
| **`GET /analytics`** | - | ✓ | ✓ | ✓ | Client |
| **`GET /analytics/documents`** | - | ✓ | ✓ | ✓ | Client |
| **`GET /analytics/export`** | - | ✓ | ✓ | ✓ | Client |
| **`GET/POST /documents/{id}/progress`** | - | ✓ | ✓ | ✓ | Client |
| **`POST /documents/{id}/verify-password`** | - | ✓ | ✓ | ✓ | Client |
| **`POST /documents/{id}/download`** | - | ✓ | ✓ | ✓ | Client |
| **`POST /documents/{id}/expiring-link`** | - | ✓ | ✓ | ✓ | Client |
| **`GET /documents/{id}/expiring-link/{token}`** | - | ✓ | ✓ | ✓ | Client |
| **`GET /categories`** | - | ✓ | ✓ | ✓ | Client |
| **`GET /tags`** | - | ✓ | ✓ | ✓ | Client |
| **`POST /bulk/import`** | - | ✓ | ✓ | ✓ | - |
| **`GET /bulk/import/status`** | - | ✓ | ✓ | ✓ | - |

**Note:** React "Client" means the `@pdf-embed-seo/react` package includes an API client that connects to WordPress or Drupal backends.

### Statistics & Analytics

| Feature | Free | Premium | WP | Drupal | React |
|---------|:----:|:-------:|:--:|:------:|:-----:|
| Basic View Counter | ✓ | ✓ | ✓ | ✓ | ✓ |
| View Count Display | ✓ | ✓ | ✓ | ✓ | ✓ |
| **Analytics Dashboard** | - | ✓ | ✓ | ✓ | ✓ |
| **Detailed View Tracking** | - | ✓ | ✓ | ✓ | ✓ |
| **Download Tracking** | - | ✓ | ✓ | ✓ | ✓ |
| **IP Address Logging** | - | ✓ | ✓ | ✓ | - |
| **IP Anonymization (GDPR)** | - | ✓ | ✓ | ✓ | - |
| **User Agent Tracking** | - | ✓ | ✓ | ✓ | - |
| **Referrer Tracking** | - | ✓ | ✓ | ✓ | - |
| **Time Spent Tracking** | - | ✓ | ✓ | ✓ | ✓ |
| **Popular Documents Report** | - | ✓ | ✓ | ✓ | ✓ |
| **Recent Views Log** | - | ✓ | ✓ | ✓ | ✓ |
| **Analytics Export (CSV/JSON)** | - | ✓ | ✓ | ✓ | ✓ |
| **Time Period Filters** | - | ✓ | ✓ | ✓ | ✓ |

### Security & Access

| Feature | Free | Premium | WP | Drupal | React |
|---------|:----:|:-------:|:--:|:------:|:-----:|
| Nonce/CSRF Protection | ✓ | ✓ | ✓ | ✓ | ✓ |
| Permission System | ✓ | ✓ | ✓ | ✓ | - |
| Capability/Access Checks | ✓ | ✓ | ✓ | ✓ | - |
| Secure PDF URL (no direct links) | ✓ | ✓ | ✓ | ✓ | ✓ |
| Input Sanitization | ✓ | ✓ | ✓ | ✓ | ✓ |
| Output Escaping | ✓ | ✓ | ✓ | ✓ | ✓ |
| **Password Protection** | - | ✓ | ✓ | ✓ | ✓ |
| **Password Hashing (bcrypt)** | - | ✓ | ✓ | ✓ | - |
| **Session-Based Access** | - | ✓ | ✓ | ✓ | ✓ |
| **Session Cache Context** | - | ✓ | - | ✓ | - |
| **Brute-Force Protection** | - | ✓ | ✓ | ✓ | - |
| **Rate Limiting** | - | ✓ | ✓ | ✓ | - |
| **Login Requirement Option** | - | ✓ | ✓ | ✓ | - |
| **Role Restrictions** | - | ✓ | ✓ | ✓ | - |
| **Expiring Access Links** | - | ✓ | ✓ | ✓ | ✓ |
| **Time-Limited URLs** | - | ✓ | ✓ | ✓ | ✓ |
| **Max Uses per Link** | - | ✓ | ✓ | ✓ | ✓ |

### Reading Experience

| Feature | Free | Premium | WP | Drupal | React |
|---------|:----:|:-------:|:--:|:------:|:-----:|
| Page Navigation | ✓ | ✓ | ✓ | ✓ | ✓ |
| Zoom Controls | ✓ | ✓ | ✓ | ✓ | ✓ |
| Full Screen Mode | ✓ | ✓ | ✓ | ✓ | ✓ |
| **Reading Progress Tracking** | - | ✓ | ✓ | ✓ | ✓ |
| **Resume Reading Feature** | - | ✓ | ✓ | ✓ | ✓ |
| **Page Position Save** | - | ✓ | ✓ | ✓ | ✓ |
| **Scroll Position Save** | - | ✓ | ✓ | ✓ | ✓ |
| **Zoom Level Save** | - | ✓ | ✓ | ✓ | ✓ |

### Developer Features

| Feature | Free | Premium | WP | Drupal | React |
|---------|:----:|:-------:|:--:|:------:|:-----:|
| WordPress Hooks (actions/filters) | ✓ | ✓ | ✓ | - | - |
| Drupal Hooks (alter/events) | ✓ | ✓ | - | ✓ | - |
| React Hooks API | ✓ | ✓ | - | - | ✓ |
| TypeScript Support | ✓ | ✓ | - | - | ✓ |
| Next.js Integration | ✓ | ✓ | - | - | ✓ |
| Template Overrides | ✓ | ✓ | ✓ | ✓ | - |
| CSS Classes for Styling | ✓ | ✓ | ✓ | ✓ | ✓ |
| JavaScript Events | ✓ | ✓ | ✓ | ✓ | ✓ |
| Cache Tags & Contexts | ✓ | ✓ | ✓ | ✓ | - |
| PdfProvider Context | ✓ | ✓ | - | - | ✓ |
| **Analytics Tracker Service** | - | ✓ | ✓ | ✓ | ✓ |
| **Progress Tracker Service** | - | ✓ | ✓ | ✓ | ✓ |
| **Rate Limiter Service** | - | ✓ | ✓ | ✓ | - |
| **Access Token Storage Service** | - | ✓ | ✓ | ✓ | - |
| **Premium Hooks & Filters** | - | ✓ | ✓ | ✓ | ✓ |
| **Priority Support** | - | ✓ | ✓ | ✓ | ✓ |

---

## React/Next.js Package Details

### NPM Packages

| Package | Version | License | Description |
|---------|---------|---------|-------------|
| `@pdf-embed-seo/core` | 1.2.11 | MIT | Core types, utilities, API client |
| `@pdf-embed-seo/react` | 1.2.11 | MIT | Free React components |
| `@pdf-embed-seo/react-premium` | 1.2.11 | Commercial | Premium React components |

### Free Components

| Component | Description |
|-----------|-------------|
| `PdfViewer` | PDF viewer with PDF.js |
| `PdfArchive` | Document listing/grid |
| `PdfSeo` | SEO schema component |
| `PdfProvider` | Context provider |

### Free Hooks

| Hook | Description |
|------|-------------|
| `usePdf` | Fetch single document |
| `usePdfList` | Fetch document list |
| `usePdfViewer` | Viewer state management |
| `useProgress` | Reading progress (requires premium backend) |
| `usePdfSettings` | Get public settings |
| `usePdfContext` | Access provider context |

### Premium Components

| Component | Description |
|-----------|-------------|
| `PdfPasswordModal` | Password protection UI |
| `PdfProgressBar` | Reading progress bar |
| `PdfSearch` | In-document search |
| `PdfBookmarks` | Bookmark navigation |
| `PdfAnalytics` | Analytics dashboard |

### Premium Hooks

| Hook | Description |
|------|-------------|
| `useAnalytics` | Analytics tracking |
| `usePassword` | Password verification |
| `useSearch` | In-document search |
| `useBookmarks` | PDF bookmark navigation |

---

## Platform Requirements

### WordPress

| Requirement | Version |
|-------------|---------|
| WordPress | 5.8+ |
| PHP | 7.4+ |
| Optional: Yoast SEO | Latest |
| Optional: ImageMagick/Ghostscript | For thumbnails |

### Drupal

| Requirement | Version |
|-------------|---------|
| Drupal | 10 or 11 |
| PHP | 8.1+ |
| Core modules | node, file, taxonomy, path, path_alias, media |
| Optional: ImageMagick/Ghostscript | For thumbnails |
| Optional: Pathauto | For auto path aliases |

### React/Next.js

| Requirement | Version |
|-------------|---------|
| Node.js | 18+ |
| React | 18+ or 19+ |
| Next.js (optional) | 13, 14, or 15 |
| TypeScript (recommended) | 5+ |
| Backend | WordPress or Drupal with plugin installed |

---

## Premium Pricing Tiers

| Feature | Starter | Professional | Agency |
|---------|:-------:|:------------:|:------:|
| **Price** | $49/year | $99/year | $199/year |
| **Sites** | 1 site | 5 sites | Unlimited |
| **Updates** | 1 year | 1 year | 1 year |
| **Support** | Email | Priority Email | Priority + Chat |
| | | | |
| Analytics Dashboard | ✓ | ✓ | ✓ |
| Password Protection | ✓ | ✓ | ✓ |
| Detailed View Tracking | ✓ | ✓ | ✓ |
| Download Tracking | ✓ | ✓ | ✓ |
| | | | |
| Reading Progress | - | ✓ | ✓ |
| XML Sitemap | - | ✓ | ✓ |
| Categories & Tags | - | ✓ | ✓ |
| AI/GEO/AEO Optimization | - | ✓ | ✓ |
| CSV/JSON Export | - | ✓ | ✓ |
| Expiring Access Links | - | ✓ | ✓ |
| | | | |
| Role-Based Access | - | - | ✓ |
| Bulk Import (CSV/ZIP) | - | - | ✓ |
| Full REST API | - | - | ✓ |
| White-Label Options | - | - | ✓ |

---

## Get Premium

**[https://pdfviewer.drossmedia.de](https://pdfviewer.drossmedia.de)**

---

## Related Documentation

- [Pro Features Overview](./PRO.md)
- [Free vs Premium Comparison](./COMPARISON.md)
- [Complete Documentation](./DOCUMENTATION.md)
- [React/Next.js Guide](./CLAUDE-REACT.md)
- [Changelog](./CHANGELOG.md)
- [WordPress Changelog](./CHANGELOG-WORDPRESS.md)
- [Drupal Changelog](./CHANGELOG-DRUPAL.md)
- [React Changelog](./CHANGELOG-REACT.md)

---

*Made with love by [Dross:Media](https://dross.net/media/)*
